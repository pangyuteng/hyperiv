...
COLUMNS = ['date', 'forward_price', 'tau', 'risk_free_rate', 'is_call', 'strike_price', 'option_price', 'log_moneyness', 'implied_volatility', 'delta', 'time_to_maturity']

ROOT_FOLDER = "/mnt/hd1/data/uw-options-cache/SPX"

def gen_data(dstamp):
    ticker = "SPXW"
    pq_file = os.path.join(ROOT_FOLDER,f"{dstamp}.parquet.gzip")
    df = pd.read_parquet(pq_file)
    print(list(df.underlying_symbol.unique()))
    print(df.shape)
    ...
    if False:
        ...
        if False: 
            # MAYBE TODO: alternatively resample to higher frequency
            # and make full option chain dataset.
            #df['tstamp_hr'] = df.tstamp_sec.apply(lambda x:x.replace(second=0,minute=0))
            df = df.set_index('date')
            df = df.groupby(['strike_price', 'is_call']).resample('1h',include_groups=False).last()
            df = df.reset_index()
        # NOTE: BELOW LOGIC NEEDED
        # just not like that.
        # we need to resample data every 5 sec
        # grab last IV for each strike and expiry
        # drop if sample size too small or ffill.

        # old NOTE: probably we want this? but we can't, as 
